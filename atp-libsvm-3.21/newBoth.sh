...
do 
	########################   split data  ########################
	python ./tools/subset.py "./matlab/bothNoiseData/${fileName}${i}" ${trainingDataSize} "./runtimeData/${fileName}" "./runtimeData/${fileName}.t"
	########################   hinge  ########################
	./CVhinge.sh "./runtimeData/${fileName}" > "./CVforBigtable/both/${fileName}_hinge"
	bestParaHinge=$(python MaxCVHinge.py "./CVforBigtable/both/${fileName}_hinge"|head -1)
	bestCHinge=$(echo $bestParaHinge|cut -d \; -f 1)
	bestGammaHinge=$(echo $bestParaHinge|cut -d \; -f 2)
	########################  ramp   ########################
	./CVramp.sh "./runtimeData/${fileName}" > "./CVforBigtable/both/${fileName}_ramp"
	bestParaRamp=$(python MaxCVRampAndPinball.py "./CVforBigtable/both/${fileName}_ramp"|head -1)
	bestCRamp=$(echo $bestParaRamp|cut -d \; -f 1)
	bestTRamp=$(echo $bestParaRamp|cut -d \; -f 2)
	bestGammaRamp=$(echo $bestParaRamp|cut -d \; -f 3)
	########################  pinball   ########################
	./CVpinball.sh "./runtimeData/${fileName}" > "./CVforBigtable/both/${fileName}_pinball"
	bestParaPinball=$(python MaxCVRampAndPinball.py "./CVforBigtable/both/${fileName}_pinball"|head -1)
	...
	########################  atp    ########################
	./CVATP.sh "./runtimeData/${fileName}" > "./CVforBigtable/both/${fileName}_atp"
	bestParaATP=$(python MaxCVATP.py "./CVforBigtable/both/${fileName}_atp"|head -1)
