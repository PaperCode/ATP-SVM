...
*/
void  cccp_subproblem(const problem *prob, schar *y, 
		double *QD, double *L, double *U, 		
		double *w, double *alpha){
	int l = prob->l;
	int max_iter_inner = 10000;//inner iteration of cccp
	int i, s, iter= 0;
	double eps_inner = 0.1;
	double d, G, tmp_L, tmp_U;
	int *index = new int[l];
	int active_size = l;
...
/*
	...
	for(i=0; i<l; i++)
	{
		...
		sparse_operator::axpy(y[i]*alpha[i], xi, ww);
	}
	for(i=0; i<w_size; i++)
	{	
		info("\n%d %lf  %lf  %d\n",i, w[i], ww[i], w[i] == ww[i]);
...
*/
	...
	while (iter < max_iter_inner)
	{
		...
		//info("%lf ",G);
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				alpha[i] = min(max(alpha[i] - G/QD[i], tmp_L), tmp_U);
		...
		if(PGmax_new - PGmin_new <= eps_inner)
		{
			if(active_size == l)
			{	
				break;
	...
	//info("\noptimization of inner loop finished, #iter = %d\n",iter);
	//info(" %d ",iter);	
	if (iter >= max_iter_inner)
		info("\nWARNING: reaching max number of iterations\n");

	delete [] index;
}

double compute_modified_pin_loss(double s, double t, double tau, double u)
{
	return max((1+tau)*u, 0.0) - max(tau*(u+s), 0.0) - max(u-t, 0.0) +tau*s;
}

static void solve_l2r_pin_svc(
	const problem *prob, double *w, double eps,
//	double Cp, double Cn,//temporarily, we don't consider unbalanced data
	double C,//Cp and Cn are the same in this part Cp = Cn = C
	double para_s, double para_t, double tau)
{
/*
	info("\ns%f\n",para_s);
	info("\nt%f\n",para_t);
...
*/
	...
	double *QD = new double[l];//the diagonal of Q
	//double *delta = new double[l];//delta_k
	double *L = new double[l];//upper bounds of constraints
	double *U = new double[l];//lower bounds of constraints
	double  yi_wk_xi = 0;//temp varaiable
	schar *y = new schar[l];
...
//for noise
	...
	{
		//start=clock();
		cccp_subproblem(prob, y, QD, L, U, w, alpha);
  		//stop=clock();
		...
		for(i=0; i<l; i++)
		{
			feature_node * const xi = prob->x[i];
			const schar yi = y[i];
			yi_wk_xi = yi*sparse_operator::dot(w, xi);
			func_value_new += C*compute_modified_pin_loss(para_s, para_t, tau, 1 - yi_wk_xi);
			//compute lower and upper bounds of constraints
			if(yi_wk_xi < 1 - para_t)
			{
				L[i] = -C*(1+tau);
				U[i] = 0.0;
				//info(" %lf", U[i]);
			}
			else if (yi_wk_xi > 1 + para_s)
			{
				L[i] = 0.0;
				U[i] = C*(1+tau);
				//info(" %lf", U[i]);
...
*/
		if (iter > 1){
			if(fabs((func_value_new - func_value_old)/func_value_1st) < eps){
				break;
			}
		}
	}
	info("CCCP outer optimization finished, #outer iter = %d\n",iter);
	//info(" %d",iter);
	if (iter >= max_iter_outer)
		info("\nWARNING: reaching max number of iterations\n");
	//info("Objective value = %lf\n", func_value_new);
	//info(" %lf ", func_value_new);
	// count SVs
/*	int nSV = 0;
	for(i=0; i<l; i++)
	{
...
* -------------------my modification begin---------------------------
*/
	double para_s = param->s;
	double para_t = param->t;
	double para_tau = param->tau;
	clock_t start, stop;
	start=clock();
/*
* -------------------my modification end---------------------------
...
* -------------------my modification begin---------------------------
*/
		case L2R_MODIFIEDPINLOSS_SVC_DUAL:
		{
			...
			solve_l2r_pin_svc(prob, w, eps, Cp, para_s, para_t, para_tau);
			break; 
		}
/*
* -------------------my modification end---------------------------
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_LR
		...
		&& param->solver_type != L2R_MODIFIEDPINLOSS_SVC_DUAL//my modification
